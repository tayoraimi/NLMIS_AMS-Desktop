...
public class TMCService {
	...
	public ObservableList<LabelValueBean> getDropdownList() {
		...
		try {
			...
			System.out.println("An error occured while getting TMC_Type_Name list, to add in combobox, error: "
		+ ex.getMessage());
	...
	public ObservableList<TMCBean> getTMCList() {
		ObservableList<TMCBean> categoryData = FXCollections.observableArrayList();
		try {
			...
			while (rs.next()) {
				TMCBean categoryBean = new TMCBean();
	...
	public boolean saveTMC(TMCBean categoryBean,String actionBtnString) throws SQLException {
		boolean flag = true;
		try {
			if (dao == null || dao.getConnection() == null || dao.getConnection().isClosed()) {
				System.out.println("In CatogaryService.validateUser() ");
			...
			flag = false;
			System.out.println("Error while saving or editing TMC, error: "
			+ e.getMessage());
			MainApp.LOGGER.setLevel(Level.SEVERE);
			MainApp.LOGGER.severe("Error while saving or editing TMC, error: "+
			MyLogger.getStackTrace(e));
		}
		return flag;
	}

	public ObservableList<TMCBean> getSearchList(
			TMCBean toSearchCatgoryBean) {
		ObservableList<TMCBean> searchData = FXCollections.observableArrayList();
		try {
			if (dao == null || dao.getConnection() == null || dao.getConnection().isClosed()) {
				System.out.println("In CatogaryService.validateUser() ");
				dao = DatabaseOperation.getDbo();
