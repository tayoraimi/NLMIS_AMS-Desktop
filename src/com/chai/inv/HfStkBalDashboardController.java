...
public class HfStkBalDashboardController {
	...
	private ObservableList<HFManBalanceDetailBean> manualHfStkBalDashboardlist = FXCollections.observableArrayList();
	...
	public void setDefaults(){
		try {	
			...
			manualHfStkBalDashboardlist=new DashboardService().getManualHfStkBalDashBoard(x_YEAR_FILTER.getValue(),x_WEEK_FILTER.getValue());
			if(manualHfStkBalDashboardlist.size()>0){
				x_GRID_PANE.getChildren().clear();
				System.out.println("in manualHfStkBalDashboardlist.size()!=0");
				...
				if((MainApp.getUserRole().getLabel().equals("SCCO")
					...
					for (int i = 0; i < manualHfStkBalDashboardlist.size(); i++) {
						System.out.println("hfNameList["+i+"]"+manualHfStkBalDashboardlist.get(i).getX_CUSTOMER_NAME()+", Product Stock Balance: "+manualHfStkBalDashboardlist.get(i).getX_STOCK_BALANCE());
						hfNameList.add(manualHfStkBalDashboardlist.get(i).getX_CUSTOMER_NAME());
					}
					...
					lgaLbl.setText(manualHfStkBalDashboardlist.get(0).getX_LGA_NAME());
...
//							blank.setStyle("-fx-background-color:red;-fx-border-color:black;");
							...
							for (int j2 = 0; j2 < manualHfStkBalDashboardlist.size(); j2++) {
								if(hfName.equals(manualHfStkBalDashboardlist.get(j2).getX_CUSTOMER_NAME())
										&& vaccineList.get(j).getLabel().equals(manualHfStkBalDashboardlist.get(j2).getX_ITEM_NUMBER())){
									System.out.println("condition1:"+hfName+"="+manualHfStkBalDashboardlist.get(j2).getX_CUSTOMER_NAME()+", condition2:"+vaccineList.get(j).getLabel()+"="+manualHfStkBalDashboardlist.get(j2).getX_ITEM_NUMBER()+", Product Stock Balance: "+manualHfStkBalDashboardlist.get(j2).getX_STOCK_BALANCE());
									TextField stkBal=new TextField();
									color="";//manualHfStkBalDashboardlist.get(j2).getX_LEGEND_COLOR();
									...
									stkBal.setText(manualHfStkBalDashboardlist.get(j2).getX_STOCK_BALANCE());
...
//                                                                        System.out.println("colour blue "+paint.toString());
	...
	public void setDashBoardPopupController(
		...
		HfStkBalDashboardController.dashboardPopupController=dashboardPopupController;
	}
